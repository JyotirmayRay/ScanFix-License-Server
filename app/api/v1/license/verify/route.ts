...
function normalizeDomain(input: string): string {
  try {
    ...
    if (clean.startsWith('http://') || clean.startsWith('https://')) clean = new URL(clean).hostname;
    clean = clean.split(':')[0];
    return clean;
  } catch {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!license) return NextResponse.json({ success: false, valid: false, reason: 'License key not found' }, { status: 404 });

    if (license.status === 'revoked') {
      await db.addLog({ licenseKey, action: 'verify', domain: cleanDomain, ip, status: 'denied', reason: 'License was revoked remotely' });
      return NextResponse.json({ success: false, valid: false, reason: 'License has been revoked by the administrator.' }, { status: 403 });
    }

    if (license.status === 'suspended') return NextResponse.json({ success: false, valid: false, reason: 'License is temporarily suspended.' }, { status: 403 });
    ...
    if (!activation && !license.allowedDomains.includes('*') && cleanDomain !== 'localhost') {
      return NextResponse.json({ success: false, valid: false, reason: `Domain '${cleanDomain}' is not activated for this license.` }, { status: 403 });
    }
    ...
    return NextResponse.json({ success: true, valid: true, token, payload });
  } catch (err: any) {
